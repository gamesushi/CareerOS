...
### 已补齐（2026-08-02 追加）
- **投递通知**：新投递通知雇主、雇主改状态通知候选人。走 BullMQ `notify` 队列
  （web 只入队，worker 发信，失败按 3 次指数退避重试）；内容构造在
  `packages/db/src/notify.ts`，发送 transport 在 `apps/worker/src/lib/mailer.ts`。
  **未配 SMTP 时静默跳过**，不会让队列任务反复失败。
- **发帖限频**：`assertPostingQuota()`，每人每 24 小时 10 个岗位（含草稿），超限 429。

### 仍未做
- **简历 PDF 渲染开销**：每次查看都实时渲染（与既有导出一致）。雇主批量看会慢，可加缓存。
- **投递不限频**：只限了发岗。同一候选人对每个岗各投一次仍可能刷屏，需要时加「每人每天最多投 N 个岗」。
- **候选人看不到自己投递的详情页**：只在岗位卡片上给状态徽标，没有独立的「我的投递」页面。
- **限频有空子**：额度按现存记录计数，删掉旧岗位能腾出额度（见 `lib/limits.ts` 注释）。
- **worker 与 web 各有一份 SMTP transport 配置**：改配置要同步两处。
